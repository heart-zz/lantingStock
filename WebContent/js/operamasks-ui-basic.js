(function($, undefined) {
    ...
    $.fn.extend({
        ...
        disableSelection: function() {
            return this.bind(($.support.selectstart ? "selectstart": "mousedown") + ".om-disableSelection",
            function(event) {
    ...
    $.omWidget("om.omDraggable", $.om.omMouse, {
        ...
        _create: function() {
            ...
            this.element.addClass("om-draggable"); (this.options.disabled && this.element.addClass("om-draggable-disabled"));
            this._mouseInit()
        },
        destroy: function() {
        ...
        _mouseStart: function(event) {
            ...
            if ($.om.ddmanager) {
                $.om.ddmanager.dragStart(this, event)
            }
            return true
        },
        ...
        _mouseStop: function(event) {
            ...
            if ((this.options.revert == "invalid" && !dropped) || (this.options.revert == "valid" && dropped) || this.options.revert === true || ($.isFunction(this.options.revert) && this.options.revert.call(this.element, dropped))) {
                var self = this;
                $(this.helper).animate(this.originalPosition, 500,
                function() {
        ...
        _mouseUp: function(event) {
            if ($.om.ddmanager) {
                $.om.ddmanager.dragStop(this, event)
            }
    ...
    $.om.plugin.add("omDraggable", "cursor", {
        onStart: function(ui, event) {
            var t = $("body"),
            o = $(this).data("omDraggable").options;
            if (t.css("cursor")) {
            ...
            if (i.scrollParent[0] != document && i.scrollParent[0].tagName != "HTML") {
                if (!o.axis || o.axis != "x") {
                    if ((i.overflowOffset.top + i.scrollParent[0].offsetHeight) - event.pageY < scrollSensitivity) {
                        ...
                        if (event.pageY - i.overflowOffset.top < scrollSensitivity) {
                            i.scrollParent[0].scrollTop = scrolled = i.scrollParent[0].scrollTop - scrollSpeed
                        }
                    }
                }
                if (!o.axis || o.axis != "y") {
                    if ((i.overflowOffset.left + i.scrollParent[0].offsetWidth) - event.pageX < scrollSensitivity) {
                        ...
                        if (event.pageX - i.overflowOffset.left < scrollSensitivity) {
                            i.scrollParent[0].scrollLeft = scrolled = i.scrollParent[0].scrollLeft - scrollSpeed
                        }
                    }
                }
                ...
                if (!o.axis || o.axis != "x") {
                    if (event.pageY - $(document).scrollTop() < scrollSensitivity) {
                        ...
                        if ($(window).height() - (event.pageY - $(document).scrollTop()) < scrollSensitivity) {
                            scrolled = $(document).scrollTop($(document).scrollTop() + scrollSpeed)
                        }
                    }
                }
                if (!o.axis || o.axis != "y") {
                    if (event.pageX - $(document).scrollLeft() < scrollSensitivity) {
                        ...
                        if ($(window).width() - (event.pageX - $(document).scrollLeft()) < scrollSensitivity) {
                            scrolled = $(document).scrollLeft($(document).scrollLeft() + scrollSpeed)
                        }
                    }
                }
    ...
    $.omWidget("om.omDroppable", {
        ...
        _drop: function(event, custom) {
            ...
            this.element.find(":data(omDroppable)").not(".om-draggable-dragging").each(function() {
                ...
                if (inst.options.greedy && !inst.options.disabled && inst.options._scope == draggable.options._scope && inst.accept.call(inst.element[0], (draggable.currentItem || draggable.element)) && $.om.intersect(draggable, $.extend(inst, {
    ...
    $.om.ddmanager = {
        ...
        drop: function(draggable, event) {
            var dropped = false;
            $.each($.om.ddmanager.droppables[draggable.options._scope] || [],
            function() {
                if (!this.options) {
                    return
                }
        ...
        dragStart: function(draggable, event) {
            ...
            function() {
                if (!draggable.options.refreshPositions) {
                    $.om.ddmanager.prepareOffsets(draggable, event)
                }
            })
        },
    ...
    $.omWidget("om.omResizable", $.om.omMouse, {
        ...
        _propagate: function(n, event) {
            $.om.plugin.call(this, n, [event, this.ui()]); (n != "resize" && this._trigger(n, event, this.ui()))
        },
    ...
    $.om.plugin.add("omResizable", "alsoResize", {
        start: function(event, ui) {
            var self = $(this).data("omResizable"),
            o = self.options;
    ...
    $.omWidget("om.sortable", $.om.omMouse, {
        ...
        _mouseDrag: function(event) {
            ...
            if ((this.overflowOffset.top + this.scrollParent[0].offsetHeight) - event.pageY < 20) {
                ...
                if (event.pageY - this.overflowOffset.top < 20) {
                    this.scrollParent[0].scrollTop = scrolled = this.scrollParent[0].scrollTop - 20
                }
            }
            if ((this.overflowOffset.left + this.scrollParent[0].offsetWidth) - event.pageX < 20) {
                ...
                if (event.pageX - this.overflowOffset.left < 20) {
                    this.scrollParent[0].scrollLeft = scrolled = this.scrollParent[0].scrollLeft - 20
                }
            }
            this.positionAbs = this._convertPositionTo("absolute");
            ...
            if ($.isArray(tools)) {
                ...
                len = tools.length; i < len; i++) {
                    ...
                    if (iconCls = this._getInnerToolCls(tool.id)) {
                        $("<div class='om-icon'></div>").addClass(iconCls).click(function(event) {
                            tool.handler.call(this, that, event)
                        }).appendTo($tool)
                    } else {
                        if (typeof tool.iconCls === "string") {
                            $("<div class='om-icon'></div>").addClass(tool.iconCls).click(function(event) {
                                tool.handler.call(this, that, event)
                            }).appendTo($tool)
                        } else {
                            if ($.isArray(tool.iconCls)) { (function(tool) {
                                    $("<div class='om-icon'></div>").addClass(tool.iconCls[0]).click(function(event) {
                                        tool.handler.call(this, that, event)
                                    }).hover(function() {
                                        if (tool.iconCls[1]) {
    ...
    $.omWidget("om.omGrid", {
        ...
        _create: function() {
            ...
            this.loadMask = $('<div class="gBlock"><div align="center" class="gBlock-valignMiddle" ><div class="loadingImg" style="display:block"/></div></div>').mousedown(function(e) {
                return false
            }).hide();
        ...
        _buildTableHead: function() {//表头
            ...
            for (var i = 0,
            len = cms.length; i < len; i++) {
                var cm = cms[i],
                cmWidth = cm.width || 60,
                cmAlign = cm.align || "center";
                if (cmWidth == "autoExpand") {
                    cmWidth = 0;
                    autoExpandColIndex = i
                }
                var thCell = $("<div></div>").html(cm.header).css({
                    "text-align": cmAlign,
                    width: cmWidth
                });
...
//                limit: limit,
            ...
            $.ajax({
                ...
                success: function(data, textStatus, request) {
                	//做修改-yjg
                	if(data.status==-1){
                		alert(data.info || '服务器操作出错,但没有返回信息');
                		return;
                	}                	
                    ...
                    self._addData(data);
                    for (var i = 0,
                    len = op._onRefreshCallbacks.length; i < len; i++) {
                        op._onRefreshCallbacks[i].call(self)
                    }
                    //self._trigger("onRefresh", null, nowPage, data.rows);
                    self._trigger("onRefresh", null, nowPage, data.list);
                    loadMask.hide();
                    self.loading = false
                },
...
//            }
          if (!this.pageData.data.list) {
            ...
            function(i, rowData) {
                ...
                $(gridHeaderCols).each(function(index) {
                    ...
                    j = 0;
                    bodyContent.push(tdTmp.replace(/\$/g,
                    function() {
                        return cols[j++]
                    }))
                });
        ...
        _buildRowCellValues: function(colModel, rowData, rowIndex) {//渲染数据
        ...
        _bindSelectAndClickEnvent: function() {
            ...
            if (!this.options.singleSelect) {
                ...
                function(event) {
                    var row = $(this),
                    index = self._getRowIndex(row);
                    if (row.hasClass("om-state-highlight")) {} else {
                        self._rowSelect(index);
                        self._refreshHeaderCheckBox()
                    }
                ...
                function(event) {
                    ...
                    index = self._getRowIndex(row);
                    if (row.hasClass("om-state-highlight")) {} else {
                        var lastSelectedIndex = self._getRowIndex(self.tbody.find("tr.om-state-highlight:not(:hidden)")); (lastSelectedIndex != -1) && self._rowDeSelect(lastSelectedIndex);
                        self._rowSelect(index)
                    }
                    self._trigger("onRowClick", event, index, self._getRowData(index))
                });
...
//            return this.pageData.data.rows[index]
        	return this.pageData.data.list[index]
        },
        ...
        _makeColsResizable: function() {
            ...
            $('th[axis^="col"] div', self.thead).omResizable({
                ...
                resize: function(ui, event) {
                    var _this = $(this),
                    abbr = _this.parent().attr("abbr"),
                    dataCells = $('td[abbr="' + abbr + '"] > div', self.tbody),
...
//            this._trigger("onRefresh", null, this.pageData.nowPage || 1, this.pageData.data.rows);
            this._trigger("onRefresh", null, this.pageData.nowPage || 1, this.pageData.data.list);
            for (var i = 0,
            len = op._onRefreshCallbacks.length; i < len; i++) {
...
//                var rowsData = self.getData().rows;
            	var rowsData = self.getData().list;
    ...
    $.omWidget("om.omBorderLayout", {
        ...
        _resizeRegion: function(init) {
            ...
            if ($northRegion) {
                ...
                $northRegion.find(">.om-panel-body").omPanel("resize", {
                    width: northWidth
                });
                $northRegion.css({
                ...
                if ($northProxy) {
                    $northProxy.outerWidth(northWidth).css({
                        left: westOpt.expandToTop ? westWidth: 0
                    })
                }
            }
            if ($southRegion) {
                ...
                $southRegion.find(">.om-panel-body").omPanel("resize", {
                    width: southWidth
                });
                $southRegion.css({
                ...
                if ($southProxy) {
                    $southProxy.outerWidth(southWidth).css({
                        left: westOpt.expandToBottom ? westWidth: 0
                    })
                }
            }
            if ($westRegion) {
                ...
                $westRegion.find(">.om-panel-body").omPanel("resize", {
                    height: westHeight
                });
                if ($westProxy) {
                    $westProxy.css({
            ...
            if ($eastRegion) {
                ...
                $eastRegion.find(">.om-panel-body").omPanel("resize", {
                    height: eastHeight
                });
                if ($eastProxy) {
                    $eastProxy.css({
            ...
            if (init) {
                ...
                if (fitCenterWidth) {
                    $centerRegion.find(">.om-panel-body").omPanel("resize", {
                        width: Math.floor(layoutWidth - westWidth - eastWidth)
                    })
                }
                if (fitEastWidth) {
                    $eastRegion.find(">.om-panel-body").omPanel("resize", {
                        width: Math.ceil(layoutWidth - westWidth - centerWidth) - this.options.spacing
                    })
                }
                if (fitWestWidth) {
                    $westRegion.find(">.om-panel-body").omPanel("resize", {
                        width: Math.ceil(layoutWidth - eastWidth - centerWidth) - this.options.spacing
                    });
                    $centerRegion.css({
                        left: $westRegion.width() + this.options.spacing
                    })
                }
            }
        },
        ...
        _createRegionProxy: function(panel, showCollapsTrigger) {
            ...
            if (showCollapsTrigger) {
                ...
                var $proxy = $(proxyHtml); (function(panel) {
            ...
            for (var i = 0; i < this.options.panels.length; i++) {
                ...
                if (panel.resizable && panel.region != "center") {
                    ...
                    if (panel.region == "west") {
                        ...
                        if (panel.region == "east") {
                            handles = "w";
                            handleClass.width = spacing;
                            handleClass.left = "-" + spacing
                        } else {
                            if (panel.region == "south") {
                                handles = "n";
                    ...
                    margin = (panel.region == "south" ? spacing: 0) + " " + (panel.region == "west" ? spacing: 0) + " " + (panel.region == "north" ? spacing: 0) + " " + (panel.region == "east" ? spacing: 0);
                    if (showCollapsTrigger) {
                        var $collapsTrigger = $("<div class='om-borderlayout-collaps-trigger-" + panel.region + "'></div>"); (function($panel) {
        ...
        collapseRegion: function(region) {
            ...
            if ($region) {
                ...
                if (panelInstance.options.closed) {
                    return
                }
    ...
    $.omWidget("om.omCalendar", {
        ...
        _render: function(o) {
            var i = 0,
            _prev, _next, _oym, $self = this.element,
            opt = this.options;
            this.ca = [];
            this._parseParam(o);
            this.con.html("");
        ...
        _buildStatusEvent: function() {
            ...
            }).mouseout(function() { ! self.isVisible() && $(this).parent().removeClass("om-state-hover")
            })
        },
        ...
        _buildParam: function() {
            ...
            opts.startDay && (opts.startDay = (7 - opts.startDay) % 7); ! opts.dateFormat && (this._defaultFormat = opts.showTime ? "yy-mm-dd H:i:s": "yy-mm-dd");
            this.EV = [];
            return this
        },
    ...
    $.extend($.om.omCalendar, {
        Page: function(config, father) {
            ...
            this._buildEvent = function() {
                ...
                cc.EV[1] = con.find("a.om-prev").bind("click",
                function(e) {
                    cc.father._monthMinus()._render();
                    return false
                });
                cc.EV[2] = con.find("a.om-next").bind("click",
                function(e) {
                ...
                cc.EV[4] = con.find(".om-setime").bind("click",
                function(e) {
                    e.preventDefault();
                    var $source = $(e.target),
                    $this = $(this);
            ...
            this.createDS = function() {
                ...
                for (i = 0; i < k; i++) {
                    ...
                    if (i < startweekday) {
                        ...
                        if ($.inArray((i + fOpts.startDay) % 7, _dis_days) >= 0) {
                            s += '<a href="javascript:void(0);" class="om-state-disabled">' + (i - startweekday + 1) + "</a>"
                        } else {
                            if (fOpts.disabledFn(_td_e) === false) {
                                s += '<a href="javascript:void(0);" class="om-state-disabled">' + (i - startweekday + 1) + "</a>"
                            } else {
                                if (fOpts.minDate instanceof Date && new Date(cc.year + "/" + (cc.month + 1) + "/" + (i + 1 - startweekday)).getTime() < (fOpts.minDate.getTime() + 1)) {
                                    s += '<a href="javascript:void(0);" class="om-state-disabled">' + (i - startweekday + 1) + "</a>"
                                } else {
                                    if (fOpts.maxDate instanceof Date && new Date(cc.year + "/" + (cc.month + 1) + "/" + (i + 1 - startweekday)).getTime() > fOpts.maxDate.getTime()) {
                                        s += '<a href="javascript:void(0);" class="om-state-disabled">' + (i - startweekday + 1) + "</a>"
                                    } else {
                                        if (i == (startweekday + (new Date()).getDate() - 1) && (new Date()).getFullYear() == cc.year && (new Date()).getMonth() == cc.month) {
                                            s += '<a href="javascript:void(0);" class="om-state-highlight om-state-nobd">' + (i - startweekday + 1) + "</a>"
                                        } else {
                                            if (i == (startweekday + fOpts.date.getDate() - 1) && cc.month == fOpts.date.getMonth() && cc.year == fOpts.date.getFullYear()) {
    ...
    $.extend($.om.omCalendar, {
        TimeSelector: function(ft, father) {
            ...
            this._timeInit = function() {
                ...
                self._render();
                self.popupannel.bind("click",
                function(e) {
                    var el = $(e.target);
                    if (el.hasClass("x")) {
                        self.hidePopup()
                    } else {
                        if (el.hasClass("om-cal-item")) {
                ...
                self.ctime.find(".u").bind("click",
                function() {
                    self.hidePopup();
                    self.add()
                });
                self.ctime.find(".d").bind("click",
                function() {
                    self.hidePopup();
                    self.minus()
                });
                self.ctime.find(".h").bind("click",
                function() {
                    var in_str = self.parseSubHtml(self.h_a);
                    self.status = "h";
                    self.showPopup(in_str)
                });
                self.ctime.find(".m").bind("click",
                function() {
                    var in_str = self.parseSubHtml(self.m_a);
                    self.status = "m";
                    self.showPopup(in_str)
                });
                self.ctime.find(".s").bind("click",
                function() {
        ...
        formatDate: function(date, formatter) {
            ...
            for (i = 0; i < formatter.length; i++) {
                ...
                if (!literal && $.omCalendar.formatCodes[fi + fi_next]) {
                    ...
                    if (!literal && $.omCalendar.formatCodes[fi]) {
    ...
    $.omWidget("om.omCombo", {
        ...
        _init: function() {
            ...
            options.disabled ? inputEl.attr("disabled", true) : inputEl.removeAttr("disabled"); (options.readOnly || !options.editable) ? inputEl.attr("readonly", "readOnly") : inputEl.removeAttr("readonly");
            if (!options.lazyLoad) {
        ...
        _bindEvent: function() {
            ...
            expandTrigger.click(function() { ! expandTrigger.hasClass("om-loading") && input.focus()
            }).mousedown(function() { ! expandTrigger.hasClass("om-loading") && span.addClass("om-state-active")
            }).mouseup(function() { ! expandTrigger.hasClass("om-loading") && span.removeClass("om-state-active")
            });
        ...
        _showDropList: function() {
            ...
            var dropListContainer = dropList.parent(),
            span = inputEl.parent();
        ...
        _highLisghtAndScrollTo: function(currentItem, targetItem) {
            ...
            if (targetItem.position().top <= 0) {
                ...
                if (targetItem.position().top + targetItem.outerHeight() > dropList.height()) {
        ...
        _backfill: function(source) {
            ...
            var value = [],
            selectedIndexs = dropList.find(".om-combo-selected");
        ...
        _filetrPass: function(filterStrategy, text, record, inputText) {
            ...
            if (typeof filterStrategy === "function") {
                ...
                if (filterStrategy === "first") {
                    ...
                    if (filterStrategy === "anywhere") {
                        return inputText.indexOf(text) > -1
                    } else {
                        if (filterStrategy === "last") {
    ...
    var uiDialogClasses = "om-dialog " + "om-widget " + "om-widget-content " + "om-corner-all ",
    sizeRelatedOptions = {
        buttons: true,
        height: true,
        maxHeight: true,
    ...
    $.omWidget("om.omDialog", {
        ...
        _create: function() {
            ...
            uiDialogTitlebarClose = $('<a href="#"></a>').addClass("om-dialog-titlebar-close " + "om-corner-tr").attr("role", "button").hover(function() {
        ...
        close: function(event) {
            var self = this,
            maxZ, thisZ, options = this.options,
        ...
        open: function() {
            ...
            self.moveToTop(true);
            if (options.modal) {
                uiDialog.bind("keypress.om-dialog",
                function(event) {
        ...
        _position: function(position) {
            ...
            if (position) {
                ...
                position = $.extend({},
                $.om.omDialog.prototype.options.position, position)
            } else {
    ...
    $.extend($.om.omDialog.overlay, {
        ...
        create: function(dialog) {
            if (this.instances.length === 0) {
                setTimeout(function() {
                    if ($.om.omDialog.overlay.instances.length) {
                        $(document).bind($.om.omDialog.overlay.events,
                        function(event) {
            ...
            var $el = (this.oldInstances.pop() || $("<div></div>").addClass("om-widget-overlay")).appendTo(document.body).css({
                width: this.width(),
                height: this.height()
            });
            if ($.fn.bgiframe) {
        ...
        resize: function() {
            var $overlays = $([]);
            $.each($.om.omDialog.overlay.instances,
            function() {
...
var swfobject = function() {
    ...
    $.omWidget("om.omFileUpload", {
        options: {
            ...
            //fileExt: "*.*",
            fileExt: "*.jpg;*.png;*.gif;*.jpeg;*.bmp;*.pdf;*.;*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx",
            fileDesc: '图片,pdf,office文件允许上传',
            sizeLimit: null,
            onSelect: function() {},
        ...
        upload: function(index) {
            ...
            if (typeof(index) != "undefined") {
                if ((fileId = _findIDByIndex(id, index)) === false) {
                    return
                }
            }
        ...
        cancel: function(index) {
            ...
            if (typeof(index) != "undefined") {
                if (isNaN(index)) {
                    ...
                    if ((fileId = _findIDByIndex(element.attr("id"), index)) === false) {
                        return
                    }
                }
        ...
        _create: function() {
            ...
            function(event, ID, fileObj) {
                if (self._trigger("onSelect", event, ID, fileObj) !== false) {
                    ...
                    $(queue).append('<div id="' + $(this).attr("id") + ID + '" class="om-fileupload-queueitem">                            <div class="cancel" onclick="$(\'#' + $(this).attr("id") + "').omFileUpload('cancel','" + ID + '\')">                            </div>                            <span class="fileName">' + fileName + " (" + byteSize + suffix + ')</span><span class="percentage"></span>                            <div class="om-fileupload-progress">                                <div id="' + $(this).attr("id") + ID + 'ProgressBar" class="om-fileupload-progressbar"><!--Progress Bar--></div>                            </div>                        </div>')
                }
            });
            element.bind("omFileUploadSelectOnce", {
            ...
            element.bind("omFileUploadQueueFull", {
            ...
            element.bind("omFileUploadCancel", {
            ...
            function(event, ID, fileObj, data, remove, clearFast) {
                if (self._trigger("onCancel", event, ID, fileObj, data) !== false) {
                    if (remove) {
                        ...
                        $("#" + $(this).attr("id") + ID).fadeOut(fadeSpeed,
                        function() {
                            $(this).remove()
                        })
                    }
            ...
            element.bind("omFileUploadClearQueue", {
            ...
            function(event, clearFast) {
                ...
                if (self._trigger("onClearQueue", event, clearFast) !== false) {
                    $("#" + queueID).find(".om-fileupload-queueitem").each(function() {
                        ...
                        $(this).delay(index * 100).fadeOut(250,
                        function() {
                            $(this).remove()
                        })
                    })
            ...
            var errorArray = [];
            element.bind("omFileUploadError", {
                ...
                if (self._trigger("onProgress", event, ID, fileObj, data) !== false) {
                    ...
                    function() {
                        if (data.percentage == 100) {
                            $(this).closest(".om-fileupload-progress").fadeOut(250,
                            function() {
                                $(this).remove()
                            })
                        }
            ...
            element.bind("omFileUploadComplete", {
            ...
            function(event, ID, fileObj, response, data) {
                if (self._trigger("onComplete", event, ID, fileObj, unescape(response), data) !== false) {
                    $("#" + $(this).attr("id") + ID).find(".percentage").text(" - 上传成功");
//                    if (settings.removeCompleted) {
//                        $("#" + $(event.target).attr("id") + ID).fadeOut(250,
//                        function() {
//                            $(this).remove()
//                        })
//                    }
            ...
            if (typeof(settings.onAllComplete) == "function") {
                element.bind("omFileUploadAllComplete", {
        ...
        $.extend(this, {
            ...
            getChanges: function(type) {
                ...
                if (reqType === "update") {
                    ...
                    for (i in uData) {
                        own.call(uData, i) && data[reqType].push($.extend(true, {},
                        this._rowIdDataMap[i], uData[i]))
                    }
            ...
            insertRow: function(index, rowData, forceAdd) {
                ...
                this._getHeaderCols().each(function(i) {
                    ...
                    j = 0;
                    trContent.push(tdTmp.replace(/\$/g,
                    function() {
                        return cols[j++]
                    }))
                });
...
//                    result.rows = [];
    ...
    function _showEditView(self, tr) {
        ...
        if ($editView.length == 0) {
            $editView = $("<div class='grid-edit-view'><div class='body-wrapper'><div class='grid-edit-row'><form class='grid-edit-form'></form></div>" + "<div class='gird-edit-btn'><input type='button' class='ok' value='确定'/><input type='button' class='cancel' value='取消'/></div></div></div>").width($elem.outerWidth()).insertAfter($elem);
    ...
    function _getCompValue(self, $tr, comp) {
        var value, rowData = _getRowData(self, $tr),
        $ins = comp.instance;
        switch (comp.type) {
        case "omCalendar":
            value = $ins.val();
    ...
    function _bindValidation(self) {
        ...
        function(index, model) {
            ...
            if (customRules) {
                ...
                for (var i = 0,
                len = customRules.length; i < len; i++) {
    ...
    function() {
        var self = this,
        ops = this.options,
        rdp = ops.rowDetailsProvider,
        cm = ops.colModel,
        $thead = this.thead;
        if (! (rdp && cm.length > 0)) {
            return
        }
        $thead.find("th:first").before('<th align="center" axis="expenderCol" class="expenderCol" rowspan=' + ($.isArray(cm[0]) ? cm.length: 1) + '><div style="text-align: center; width: 14px;"></div></th>');
        ...
        this.tbody.delegate("td.expenderCol >div", "click",
        function(event) {
            var _this = $(this),
    ...
    function() {
        ...
        for (var i = 0,
        len = pagerNvg.length; i < len; i++) { (function(i) {
    ...
    $.omWidget("om.omMenu", {
        ...
        _bindEvent: function() {
            ...
            for (var j = 0; j < uls.length; j++) {
                $(uls[j]).bind("mouseleave.menuContainer",
                function() {
                    var ul = $(this);
        ...
        _selectPrev: function() {
            ...
            if (menuItemHover.length == 0) {
                var lastLi = curLi.eq(curLi.length - 1),
                i = 1;
    ...
    $.omWidget("om.omScrollbar", {
        ...
        _mousewheelListener: function(event) {
            ...
            barCurTop = $bar.position().top - elemTop,
        ...
        _setTimer: function(type) {
            var self = this;
            clearTimeout(this._getScrollbar(type)._timer); (function(type) {
                setTimeout(function() {
    ...
    var tabIdPrefix = "om-tabs-" + (((1 + Math.random()) * 65536) | 0).toString(16).substring(1) + "-",
    id = 0;
    ...
    $.omWidget("om.omTabs", {
        ...
        _buildEvent: function() {
            ...
            $closeIcon.unbind("click.omtabs").bind("click.omtabs",
            function(e) {
            ...
            if (options.switchMode.indexOf("mouseover") != -1) {
                $tabInner.bind("mouseover.omtabs",
                function() {
                    var tabId = $(this).attr("tabId"),
                    timer = $.data($self, "activateTimer"); (typeof timer !== "undefined") && clearTimeout(timer);
                    timer = setTimeout(function() {
                ...
                if (options.switchMode.indexOf("click") != -1) {
                    $tabInner.bind("click.omtabs",
                    function() {
            ...
            if (options.switchMode.indexOf("mouseover") == -1) {
                ...
                $lis.bind("mouseover.omtabs",
                function() {
                    addState("hover", $(this))
                });
                $lis.bind("mouseout.omtabs",
                function() {
                    removeState("hover", $(this))
                });
                if (options.closable && options.tabMenu) {
                    $lis.each(function(index, item) {
                        $(item).bind("contextmenu",
                        function(e) {
                            if ($.fn.omMenu) {
                                that.$currentLi = this;
            ...
            $self.find(">div.om-tabs-headers >span").bind("click.omtabs",
            function(e) {
        ...
        _close: function(index) {
            ...
            if ($panels.children().length == 0) {
                $panels.css({
                    height: omtabsHeight - $headers.outerHeight()
                })
            }
    ...
    $.omWidget("om.omTooltip", {
        ...
        _bindEvent: function() {
            ...
            if (options.showOn == "mouseover") {
                element.bind("mouseover.tooltip",
                function(e) {
                    if (self.showTime) {
                ...
                if (options.showOn == "click") {
                    self.showTime = element.bind("click.tooltip",
                    function(e) {
                        setTimeout(function() {
                            self.show(e)
                        },
            ...
            if (options.trackMouse) {
                element.bind("mousemove.tooltip",
                function(e) {
                    self._adjustPosition(e)
                })
            }
            element.bind("mouseleave.tooltip",
            function() {
                if (self.showTime) {
                    clearTimeout(self.showTime)
                }
        ...
        _adjustPosition: function(event) {
            ...
            options = self.options,
            element = self.element,
            top, left, offSet = $(element).offset();
