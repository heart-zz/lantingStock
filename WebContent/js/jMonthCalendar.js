...
(function($) {
	...
	var defaults = {
			...
			onDayLinkClick: function(date,recordId) { return true; },
			onDayCellClick: function(date,recordId) { return true; },
	...
	var getDataFromServer=function(url){//根据日期获取数据
		return eval("("+$.ajax({
			url:url,
			type:'post',
			async:false,
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				var atts={'class':'','date':'','id':'','recordId':''};
				if (curDay < 0 || curDay >= maxDays) {
					isActive=false;
				} else {
					d.setDate(curDay+1);
				}
				atts['class']+=" DateBox" + (weekday == 0 || weekday == 6 ? ' Weekend ' : '');
				atts['date']=_currentDate;
				atts['id']=getDateId(_currentDate);
				atts['recordId']=_data[curDay+1];
				...
				//DateBox Events 具体日期事件绑定
				var dateLink='';
				if(isActive)
					if(_data[_currentDate.getDate()]>0){
						dateLink = jQuery('<a href="#" title="已有日志">&#10004;' + _currentDate.getDate() + '</a>').click(function(e) {
		                    //defaults.onDayLinkClick($(this).parent().attr("date"));
							defaults.onDayLinkClick($(this).parent().attr("date"),$(this).parent().attr("recordId"));
		                    e.stopPropagation();
		                });
					}	
				else
					dateLink = jQuery('<a href="#" title="没有日志">' + _currentDate.getDate() + '</a>').click(function(e) {
	                    defaults.onDayLinkClick($(this).parent().attr("date"),0);
	                    e.stopPropagation();
	                });
				var dateBox = jQuery("<td></td>").attr(atts).append(dateLink).dblclick(function(e) {
                    defaults.onDayCellDblClick($(this).attr("date"));
                    e.stopPropagation();
                }).click(function(e) {
					defaults.onDayCellClick($(this).attr("date"));
                    e.stopPropagation();
				});
	...
	jQuery.J.ChangeMonth = function(dateIn) {
		_data=getDataFromServer('getWorkLogStatus?date='+dateIn.toString("yyyy-MM-dd"));//从服务器获取数据
		defaults.onMonthChanging(dateIn);
		jQuery.J.DrawCalendar(dateIn);
		defaults.onMonthChanged(dateIn);
	};
	
	jQuery.J.Initialize = function(options, events,data) {
		_data=data || [];
